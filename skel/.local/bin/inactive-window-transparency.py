...
# Define apps that should never be dimmed (app_id for Wayland, window_class for XWayland)
EXCLUDED_APPS = ["firefox", "mpv"]

def is_excluded(window):
    """Check if the window matches any excluded app_id or window_class."""
    if window is None:
        return False
    return window.app_id in EXCLUDED_APPS or window.window_class in EXCLUDED_APPS
...
def on_window(args, ipc, event):
    ...
    for window_id in focused_set:
        ...
        if window is None:
            ...
            # Check exclusions before applying inactive opacity
...
if __name__ == "__main__":
    ...
    for window in ipc.get_tree():
        if window.focused:
            ...
            # Check exclusions during initial startup loop
            if is_excluded(window):
                window.command("opacity " + args.focused)
            else:
                window.command("opacity " + args.opacity)
